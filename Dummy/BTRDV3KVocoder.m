...
NSString * const BTRVocoderDeviceChanged = @"BTRVocoderDeviceChanged";
...
static void VocoderAdded(void *refCon, io_iterator_t iterator) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 5ull * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
        ...
        if(ports.count == 1) {
            [BTRDataEngine sharedInstance].vocoder.serialPort = ports[0];
            [[BTRDataEngine sharedInstance].vocoder start];
        }
        
        [[NSNotificationCenter defaultCenter] postNotificationName:BTRVocoderDeviceChanged object: nil];
    });
}
...
static void VocoderRemoved(void *refCon, io_iterator_t iterator) {
    ...
    while((serialDevice = IOIteratorNext(iterator))) {
        ...
        if([deviceFile isEqualToString:[BTRDataEngine sharedInstance].vocoder.serialPort])
            dispatch_async(dispatch_get_main_queue(), ^{
                [[BTRDataEngine sharedInstance].vocoder stop];
            });
    };
